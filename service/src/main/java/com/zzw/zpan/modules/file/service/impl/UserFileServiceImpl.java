...
import com.zzw.zpan.StorageEngine;
import com.zzw.zpan.common.event.search.UserSearchEvent;
import com.zzw.zpan.common.stream.event.file.DeleteFileEvent;
import com.zzw.zpan.common.utils.HttpUtil;
import com.zzw.zpan.common.utils.UserIdUtil;
import com.zzw.zpan.constants.RPanConstants;
import com.zzw.zpan.context.ReadFileContext;
...
import com.zzw.zpan.modules.file.enums.enums.FileTypeEnum;
import com.zzw.zpan.modules.file.enums.enums.FolderFlagEnum;
import com.zzw.zpan.modules.file.mapper.RPanUserFileMapper;
import com.zzw.zpan.modules.file.service.IFileChunkService;
import com.zzw.zpan.modules.file.service.IFileService;
...
public class UserFileServiceImpl extends ServiceImpl<RPanUserFileMapper,RPanUserFile>  implements IUserFileService, ApplicationContextAware {
    ...
    @Resource
    OSSStorageEngine storageEngine;
    ...
    public FileChunkUploadVO chunkUpload(FileChunkUploadContext context) {
        FileChunkSaveContext fileChunkSaveContext = context.asViewObject(FileChunkSaveContext.class,o->{
    ...
    public List<RPanUserFileVO> transferVOList(List<RPanUserFile> records) {
        ...
        return records.stream().map(o->{
            return o.asViewObject(RPanUserFileVO.class);
    ...
    private int getDuplicateFilename(RPanUserFile entity, String newFilenameWithoutSuffix) {
        return (int) count(Wrappers.<RPanUserFile>query()
                .eq("parent_id", entity.getParentId())
                .eq("folder_flag", entity.getFolderFlag())
                .eq("user_id", entity.getUserId())
                .eq("del_flag", DelFlagEnum.NO.getCode())
                .likeLeft("filename", newFilenameWithoutSuffix));
    }
    ...
    private void checkUpdateFilenameCondition(UpdateFilenameContext context) {
        ...
        int count = (int) count(Wrappers.<RPanUserFile>query()
                .eq("parent_id", entity.getParentId())
                .eq("filename", context.getNewFilename()));
    ...
    private void saveFile(FileUploadContext context) {
        FileSaveContext fileSaveContext = context.asViewObject(FileSaveContext.class ,o->{
            o.setFile(context.getFile());
            o.setRecord(context.getRecord());
            o.setUserId(UserIdUtil.get());
            o.setIdentifier(context.getIdentifier());
            o.setFilename(context.getFilename());
            o.setTotalSize(context.getTotalSize());
        });
    ...
    private void mergeFileChunkAndSaveFile(FileChunkMergeContext context) {
        FileChunkMergeAndSaveContext fileChunkMergeAndSaveContext = context.asViewObject(FileChunkMergeAndSaveContext.class, o -> {
            o.setUserId(context.getUserId());
            o.setFilename(context.getFilename());
            o.setRecord(context.getRecord());
            o.setParentId(context.getParentId());
            o.setTotalSize(context.getTotalSize());
            o.setIdentifier(context.getIdentifier());
        });
    ...
    private List<FolderTreeNodeVO> assembleFolderTreeNodeVOList(List<RPanUserFile> folderRecords) {
        ...
        List<FolderTreeNodeVO> mappedFolderTreeNodeVOList = folderRecords.stream().map(o-> o.asViewObject(FolderTreeNodeVO.class, v->{
           v.setParentId(o.getParentId());
           v.setLabel(o.getFilename());
           v.setId(o.getFileId());
        })).collect(Collectors.toList());
        ...
        for (FolderTreeNodeVO node : mappedFolderTreeNodeVOList) {
                ...
                if (CollectionUtils.isNotEmpty(children)) {
                    node.getChildren().addAll(children);    //实际上也会修改掉mappedFolderTreeNodeVOList这里面的属性
                }
            }
    ...
    /**
     * 拼装当前文件记录以及所有的子文件记录
     *  递归循环自己 可以把所有的一个文件夹里的方法全部加入到list中
     *  saveBatch来批次保存
     * @param allRecords
     * @param record
     * @param targetParentId
